# TODO: Make plots that high priority requests are getting serviced better: Our new claim
# TODO: Is more number of hops better or less better? More bandwidth utilization expected or less?
...
# seed = 0
# TODO:revert to og
repetitions = config.repetitions  # 33
twindow_length = 1
...
class Sim:
    ...
    def add_event(self, evt):
        ...
        if evt.tipo == "arrival":
            ...
            request = nsl_request.get_nslr(
                self.total_reqs, service_type, mean_operation_time
            )  #
...
def filtro(window_req_list, action):
    ...
    for req in window_req_list:
        if (
            (req.service_type == "embb" and req.bandera <= actions[action][0] * 100)
            or (req.service_type == "urllc" and req.bandera <= actions[action][1] * 100)
            or (req.service_type == "miot" and req.bandera <= actions[action][2] * 100)
        ):
...
def update_resources(substrate, nslr, kill):
    ...
    for vnf in nslr.nsl_graph_reduced[
        "vnodes"
    ]:  # se recorre los nodos del grafo reducido del nslr aceptado
        if "mapped_to" in vnf:
            n = next(
                n
                for n in nodes
                if (n["id"] == vnf["mapped_to"] and n["type"] == vnf["type"])
            )  #
            if vnf["type"] == 0:
        ...
        for i in range(len(path) - 1):
            try:
                l = next(
                    ...
                    for l in links
                    if (
                        (l["source"] == path[i] and l["target"] == path[i + 1])
                        or (l["source"] == path[i + 1] and l["target"] == path[i])
                    )
                )
                if kill:
                    l["bw"] += vlink["bw"]
...
def resource_allocation(cn, service_type_priority):  # cn=controller
    ...
    for req in sim.granted_req_list:
        ...
        if not rejected:
            ...
            evt = sim.create_event(
                tipo="termination", inicio=req.end_time, extra=req, f=func_terminate
            )
            sim.add_event(evt)

            # calculo de metricas (profit, acpt_rate, contadores)
            sim.accepted_reqs += 1
            reward_node, profit_nodes = (
                calculate_metrics_modified.calculate_profit_nodes(
                    req, end_simulation_time, service_type_priority
                )
            )
            rew_link, profit_links = calculate_metrics_modified.calculate_profit_links(
                req, end_simulation_time, service_type_priority
            )
            profit_links *= 10  # original code
            # Adding up rewards
            ...
            step_node_utl += (a + b) / (
                (edge_initial + centralized_initial) * end_simulation_time
            )
...
def get_state(substrate, simulation):
    ...
    cod_avble_central = get_code(
        substrate.graph["centralized_cpu"] / centralized_initial
    )
    ...
    if total == 0:
        ...
        pct_embb, pct_urllc, pct_miot = (
            simulation.current_instatiated_reqs[0] * 100 / total,
            simulation.current_instatiated_reqs[1] * 100 / total,
            simulation.current_instatiated_reqs[2] * 100 / total,
        )
    ...
    if n == 0:
        ...
        for req in simulation.granted_req_list:
            if req.service_type == "embb":
                ...
                contador[2] += 1
        pct_arriv_embb, pct_arriv_urllc, pct_arriv_miot = (
            contador[0] * 100 / n,
            contador[1] * 100 / n,
            contador[2] * 100 / n,
        )
...
def func_arrival(c, evt):  # NSL arrival
    ...
    s.add_event(
        s.create_event(
            tipo="arrival",
            inicio=s.horario + inter_arrival_time,
            extra={"service_type": service_type, "arrival_rate": arrival_rate},
            f=func_arrival,
        )
    )
...
def func_twindow(c, evt):
    ...
    evt = sim.create_event(
        tipo="twindow_end",
        inicio=sim.horario + twindow_length,
        extra={
            "first_state": False,
            "end_state": end_state,
            "current_state": s,
...
def prepare_sim(s):
    evt = s.create_event(
        tipo="arrival",
        inicio=s.horario + get_interarrival_time(embb_arrival_rate),
        extra={"service_type": "embb", "arrival_rate": embb_arrival_rate},
        f=func_arrival,
    )
    s.add_event(evt)
    evt = s.create_event(
        tipo="arrival",
        inicio=s.horario + get_interarrival_time(urllc_arrival_rate),
        extra={"service_type": "urllc", "arrival_rate": urllc_arrival_rate},
        f=func_arrival,
    )
    s.add_event(evt)
    evt = s.create_event(
        tipo="arrival",
        inicio=s.horario + get_interarrival_time(miot_arrival_rate),
        extra={"service_type": "miot", "arrival_rate": miot_arrival_rate},
        f=func_arrival,
    )
    s.add_event(evt)
    ...
    evt = s.create_event(
        tipo="twindow_end",
        inicio=s.horario + twindow_length,
        extra={"first_state": True, "end_state": False},
        f=func_twindow,
    )
    s.add_event(evt)
...
def main():
    ...
    for m in arrival_rates:
        ...
        # TODO: See if number of requests: delay can be done
        # need to plot delays for every type of serviced request
        total_delay_rep = []
        embb_delay_rep = []
        urllc_delay_rep = []
        miot_delay_rep = []
        ...
        for i in range(episodes):
            total_delay_rep.append([])
            embb_delay_rep.append([])
            urllc_delay_rep.append([])
            miot_delay_rep.append([])
        ...
        for i in range(repetitions):
            ...
            for j in range(episodes):
                ...
                total_delay_rep[j].append(controller.total_delay)
                embb_delay_rep[j].append(controller.embb_delay)
                urllc_delay_rep[j].append(controller.urllc_delay)
                miot_delay_rep[j].append(controller.miot_delay)
                ...
                acpt_rate_rep[j].append(
                    controller.simulation.accepted_reqs
                    / controller.simulation.total_reqs
                )
                acpt_rate_embb_rep[j].append(
                    controller.simulation.embb_accepted_reqs
                    / controller.simulation.total_embb_reqs
                )
                acpt_rate_urllc_rep[j].append(
                    controller.simulation.urllc_accepted_reqs
                    / controller.simulation.total_urllc_reqs
                )
                acpt_rate_miot_rep[j].append(
                    controller.simulation.miot_accepted_reqs
                    / controller.simulation.total_miot_reqs
                )
            ...
            f.write(str(embb_delay_rep) + "\n\n")
            f.write("**urllc_delay_rep:\n")
            f.write(str(urllc_delay_rep) + "\n\n")
            f.write("**miot_delay_rep:\n")
            f.write(str(miot_delay_rep) + "\n\n")
            f.close()
