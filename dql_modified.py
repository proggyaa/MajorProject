...
class Agent(object):
    ...
    def policy(self, state, training):
        # MODIFIED: Boltzmann policy for training, greedy policy otherwise
        temperature = config.temperature
        
        if training:
            inputs = np.expand_dims(state, 0)
            qvalues = self.online_network.model(inputs)
            probabilities = tf.nn.softmax(qvalues / temperature).numpy()
            action = np.random.choice(self.action_space_size, p=probabilities.squeeze())
        else:
            inputs = np.expand_dims(state, 0)
            qvalues = self.online_network.model(inputs)
